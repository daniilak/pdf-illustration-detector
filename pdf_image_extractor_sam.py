...
class PDFImageExtractorSAM:
    def __init__(self, model_path='sam_vit_h_4b8939.pth', model_type='vit_h'):
        """
        Инициализация экстрактора изображений с использованием SAM
        :param model_path: путь к модели SAM
        :param model_type: тип модели SAM (vit_h, vit_l, vit_b)
        """
        ...
        self.logger.info(f"Инициализация модели SAM: {model_type}")
        self.sam = sam_model_registry[model_type](checkpoint=model_path)
        self.predictor = SamPredictor(self.sam)
        self.logger.info("Модель SAM успешно загружена")
    ...
    def extract_images_from_pdf(self, pdf_path, output_dir, min_confidence=0.15):
        """
        Извлечение изображений из PDF с помощью SAM
        :param pdf_path: путь к PDF файлу
        :param output_dir: директория для сохранения результатов
        :param min_confidence: минимальный порог уверенности для сегментации
        """
        ...
        try:
            for page_num in range(total_pages):
                ...
                # Устанавливаем изображение для SAM
                self.predictor.set_image(img_np)
                
                # Создаем сетку точек для поиска изображений
                grid_size = 100
                points = []
                for x in range(grid_size, img_np.shape[1], grid_size):
                    for y in range(grid_size, img_np.shape[0], grid_size):
                        points.append([x, y])
                
                input_points = np.array(points)
                input_labels = np.ones(len(points))
                
                self.logger.info(f"Создано {len(points)} точек для сегментации")
                        ...
                        if contours:
                            ...
                            if self.is_valid_image_region(x1, y1, x2, y2, img_np.shape):
                                ...
                                if region.size > 0 and not self.is_text_region(region):
                                    ...
                                    image_path = images_dir / f'page_{page_num}_image_{mask_idx}.jpg'
